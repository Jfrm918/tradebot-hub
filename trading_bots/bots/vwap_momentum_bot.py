#!/usr/bin/env python3
"""
VWAP Momentum Bot — BTC strategy
Enters when price breaks through VWAP with momentum, rides the trend.
Exit on profit target, stop loss, or VWAP crossback.
...
PROFIT_TARGET    = 0.008      # exit at +0.8%
STOP_LOSS        = 0.004      # exit at -0.4%
CHECK_INTERVAL   = 30
LOG_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'logs')
LOG_FILE = os.path.join(LOG_DIR, 'vwap_mom.log')
# ──────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    position   = None
    prev_side  = None   # was price 'above' or 'below' VWAP last tick

    price_sum  = 0.0
    tick_count = 0

    log(f"VWAP Momentum Bot started | Balance: ${balance:.2f}")
    ...
    while True:
        try:
            ...
            dev       = (price - vwap) / vwap
            curr_side = 'above' if price > vwap else 'below'

            if position is None:
                # Bullish VWAP breakout: crossed above VWAP with momentum
                if prev_side == 'below' and curr_side == 'above' and dev >= BREAKOUT_PCT:
                ...
                # Bearish VWAP breakdown: crossed below VWAP with momentum
                elif prev_side == 'above' and curr_side == 'below' and dev <= -BREAKOUT_PCT:
                    size = round((balance * TRADE_FRACTION) / price, 6)
